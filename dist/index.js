  ...
  "node_modules/ncp/lib/ncp.js"(exports, module2) {
    ...
    function ncp(source, dest, options, callback) {
      ...
      function copyFile(file, target) {
        var readStream = fs2.createReadStream(file.name), writeStream = fs2.createWriteStream(target, { mode: file.mode });
        ...
        writeStream.once("finish", function() {
          if (modified) {
            fs2.utimesSync(target, file.atime, file.mtime);
            cb();
          } else
            cb();
        });
      ...
      function onLink(link) {
        ...
        fs2.readlink(link, function(err, resolvedPath) {
          if (err) {
            return onError(err);
          }
...
var require_mv = __commonJS({
  "node_modules/mv/index.js"(exports, module2) {
    var fs2 = require("fs");
    ...
    module2.exports = mv2;
    function mv2(source, dest, options, cb) {
      if (typeof options === "function") {
        cb = options;
        options = {};
      }
...
var createEnv = async ({
  ...
  const reactAppSecrets = Object.entries(secretsParse).filter(
    ([key, value]) => /REACT_APP.*/.test(key)
  );
  const envValues = {};
  for (let [key, value] of reactAppSecrets)
    envValues[key] = value;
...
var moveEnv = async (payload) => {
  ...
  console.log(output);
  import_core.default.setOutput("reactSecrets", envValues);
};
var createEnvFile = async ({
  ...
  try {
    ...
    console.error(err);
    import_core.default.setFailed("Something went wrong. Check error in logs");
  }
};
var main = async () => {
